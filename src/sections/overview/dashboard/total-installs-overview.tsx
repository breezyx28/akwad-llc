...
export function TotalIntallsOverview({ sx, ...other }: CardProps) {
  const theme = useTheme();

  const tabs = useTabs('total_use');

  const chartColors =
    tabs.value === 'total_use' ? [theme.palette.primary.dark] : [theme.palette.warning.dark];
  ...
  return (
    <Card sx={{ p: 3, ...sx }} {...other}>
      ...
      <Chart
        type="line"
        series={tabs.value === 'total_use' ? TABS[0].chart.series : TABS[1].chart.series}
